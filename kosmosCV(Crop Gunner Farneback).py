...
# FILE = 'Crop-GunnerFarneback/Crop(Second).mp4'
FILE = 'Crop-GunnerFarneback/Crop(First).mp4'
...
def drawFlow(img,flow,step=16):
    ...
    for x,y in indices:   # 인덱스 순회
        if abs(np.sum(flow[y, x].astype(np.int32))) <= 3:
            continue
...
with open('crop_gunnerFarneback.csv', 'w') as f:
    wr = csv.writer(f)
